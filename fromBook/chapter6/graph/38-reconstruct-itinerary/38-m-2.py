        ...
        for ticket in tickets:
            ...
            for (index, arrival_elem) in enumerate(arrival_list):
                if arrival_elem[0] > arrival:
                    i = index
                    break
            ticket_dict[depature].insert(i, [arrival, 0])

        result = []

        def dfs(departure: str) -> int:
            ...
            for arrival in ticket_dict[departure]:
                if arrival[1] == -1:
                    ...
                    arrival[1] + len(result) + 1 >= len(tickets) + 1:
                        arrival[1] = -1
                        arrival[1] = dfs(arrival[0])
                        depth = max(arrival[1] + 1, depth)
